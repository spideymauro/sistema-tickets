...
const NOMBRE_ADMIN = 'Mauricio';
const APELLIDO_ADMIN = 'Contreras';
const PUESTO_ADMIN = 'Administrador del Sistema';
...
function seedAdminIfNeeded(db) {
  bcrypt.hash(PASSWORD_ADMIN, 10, (err, hash) => {
    ...
    db.run(
      `INSERT OR IGNORE INTO usuarios_staff (correo, password_hash, rol, nombre, apellido, puesto)
       VALUES (?, ?, 'admin', ?, ?, ?)`,
      [CORREO_ADMIN, hash, NOMBRE_ADMIN, APELLIDO_ADMIN, PUESTO_ADMIN],
      function (err) {
