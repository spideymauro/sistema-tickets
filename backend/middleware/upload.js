...
const fs = require('fs');
const { validarExtensionPermitida } = require('../utils/listaNegraArchivos');
...
function filtroTipos(tiposPermitidos) {
  return (req, file, cb) => {
    // Capa 1: lista negra explícita por extensión (separada del allowlist de tipo MIME)
    const errorListaNegra = validarExtensionPermitida(file.originalname);
    if (errorListaNegra) {
      return cb(new Error(errorListaNegra));
    }

    // Capa 2: allowlist por tipo MIME declarado
