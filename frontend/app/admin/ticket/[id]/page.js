      ...
      <div className={styles.hilo}>
        {ticket.mensajes.map((m) => (
          ...
          >
            ...
            {ticket.adjuntos.filter((a) => a.id_mensaje === m.id_mensaje).length > 0 && (
              <div className={styles.contenedorAdjuntos}>
                {ticket.adjuntos
                  .filter((a) => a.id_mensaje === m.id_mensaje)
                  .map((a) =>
                    a.tipo_archivo?.startsWith('image/') ? (
                      <a
                        key={a.id_adjunto}
                        href={urlDescarga(a)}
                        target="_blank"
                        rel="noreferrer"
                        className={styles.miniaturaAdjunto}
                        title="Ver imagen completa"
                      >
                        <img src={urlDescarga(a)} alt="Adjunto" className={styles.imagenMiniatura} />
                      </a>
                    ) : (
                      <a
                        key={a.id_adjunto}
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={styles.tarjetaAdjuntoPdf}
                        title="Ver PDF"
                      >
                        <span className={styles.iconoPdf}>📄</span>
                        <span className={styles.etiquetaPdf}>Ver PDF</span>
                      </a>
                    )
                  )}
              </div>
