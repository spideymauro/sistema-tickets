...
import { validarExtensionPermitida } from '../lib/listaNegraArchivos';
...
export default function BotWidget() {
  ...
  function manejarArchivos(e) {
    const seleccionadosNuevos = Array.from(e.target.files || []);
    if (seleccionadosNuevos.length === 0) return;

    const combinados = [...archivos, ...seleccionadosNuevos];
    ...
    if (combinados.length > 3) {
      setErrorArchivos('Solo puedes adjuntar hasta 3 archivos en total.');
      limpiarInput();
      return;
    }

    // Capa 1: lista negra explícita por extensión, antes de cualquier otra validación
    for (const archivo of seleccionadosNuevos) {
      const errorListaNegra = validarExtensionPermitida(archivo.name);
      if (errorListaNegra) {
        setErrorArchivos(errorListaNegra);
        limpiarInput();
        return;
      }
    }

    const muyPesado = seleccionadosNuevos.find((f) => f.size > 5 * 1024 * 1024);
    if (muyPesado) {
      setErrorArchivos(`El archivo "${muyPesado.name}" pesa más de 5MB.`);
      limpiarInput();
      return;
    }
    const tipoInvalido = seleccionadosNuevos.find((f) => !TIPOS_PERMITIDOS.includes(f.type));
    if (tipoInvalido) {
      setErrorArchivos(
        `El archivo "${tipoInvalido.name}" no es un tipo permitido. Solo se aceptan imágenes (JPG, PNG, WEBP) o PDF.`
      );
      limpiarInput();
      return;
    }
    ...
    limpiarInput(); // permite volver a abrir el selector y seguir agregando (hasta el máximo de 3)
  }

  function quitarUnArchivo(indice) {
            ...
            {archivos.length > 0 && (
              <div className={styles.listaArchivos}>
                {archivos.map((archivo, i) => (
                  <div key={i} className={styles.filaArchivo}>
                    <span className={styles.iconoArchivo}>{iconoDeArchivo(archivo.type)}</span>
                    <span className={styles.nombreArchivo}>{archivo.name}</span>
                    <span className={styles.pesoArchivo}>{formatearTamano(archivo.size)}</span>
                    <button
                      className={styles.botonQuitarUno}
                      onClick={() => quitarUnArchivo(i)}
                      type="button"
                ...
                {archivos.length < 3 && (
                  <p className={styles.notaArchivos}>Puedes agregar {3 - archivos.length} más.</p>
                )}
              </div>
            )}
            {errorArchivos && (
              <div className={styles.filaArchivosListos}>
                <p className={styles.textoError}>{errorArchivos}</p>
                <button className={styles.botonQuitar} onClick={quitarArchivos} type="button">
                  Quitar
                </button>
              </div>
            )}
